...
void racetrack(void)
{
	...
	if(flag==2) drawRectangle(966.0,956.0,300.0,450.0,1.0,1.0,1.0); else drawRectangle(1266.0,1256.0,300.0,450.0,1.0,1.0,1.0);	//end line
}
...
{	
	...
	if(flag == 2) {					//display second game
		...
		if(nmo<900 )
			glTranslatef(nmo,0,0);
		else if(nmo<1180)	glTranslatef(nmo,110,0);
		else	glTranslatef(nmo,0,0);
		man(30,98); 
		glPopMatrix();
	}
